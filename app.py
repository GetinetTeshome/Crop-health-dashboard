...
def get_ndvi_tile_url(lon, lat, start, end):
    ...
    # Pull Sentinel-2 Surface Reflectance (cloud filtered)
    ...
    # Standard Plant Health formula computation: NDVI = (NIR - Red) / (NIR + Red)
    ndvi = s2.normalizedDifference(['B8', 'B4']).rename('NDVI')
    
    # Visualization hex codes (Red = Soil/Stressed, Yellow = Moderate, Green = Dense Canopy)
    ndvi_vis = {
        'min': 0.0,
        'max': 0.8,
    ...
    # Generate an API map tile URL directly from Google's server pool
...
if st.button("Generate Crop Health Analysis"):
    with st.spinner("Processing cloud assets from Sentinel-2..."):
        try:
            ...
            # Create standard OpenStreetMap base object centered on coordinates
            m = folium.Map(location=[coords[1], coords[0]], zoom_start=12, tiles="OpenStreetMap")
            
            # Overlay Google's generated satellite imagery tiles onto the open base map
            folium.TileLayer(
                tiles=tile_url,
            ...
            # Output map to your frontend canvas
